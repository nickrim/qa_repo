...
class Script38RubyTest < Test::Unit::TestCase
  ...
  def test_script38_ruby
    ...
    @a = @driver.find_element(:css, "td > h3").text
    @a = @a.reverse.chop.reverse.gsub(/\,/,"").to_f
    puts @a
    #add from 38_02
    @driver.get(@base_url + "/")
    @driver.find_element(:name, "param[homevalue]").clear
    ...
    @driver.find_element(:name, "param[pmi]").send_keys "0"
    @driver.find_element(:css, "input[type=\"submit\"]").click
    verify { assert_equal "$2,864.49", @driver.find_element(:css, "td > h3").text }
    @b = @driver.find_element(:css, "td > h3").text
    @b = @b.reverse.chop.reverse.gsub(/\,/,"").to_f
    puts @b
    @c = 100.00 - @a/@b*100
    print "percentage increase (3% vs. 4%) is ",@c.round(2)
  end
